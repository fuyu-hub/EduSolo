...
export default function Canvas2DInterativo({
  ...
  const handleDuploCliqueCanvas = (e: React.MouseEvent) => {
    ...
    if (z >= 0) {
      ...
      const zSnapped = Math.round(z / GRID_SNAP_METROS) * GRID_SNAP_METROS;
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent className="p-0">
        <div ref={containerRef} className="relative w-full h-[500px] bg-muted/30 overflow-hidden">
          <svg
            ref={svgRef}
            width="100%"
            height="100%"
        ...
        <g id="svg-mundo" transform={`translate(${origem.x}, ${origem.y})`}>
          ...
          {selectedPoints.map(pontoId => {
            ...
            return (
              <div
                key={`tooltip-${pontoId}`}
                className="absolute z-50 bg-gradient-to-br from-card to-card/95 backdrop-blur-md text-card-foreground rounded-xl shadow-2xl border-2 border-primary transition-all duration-300 ease-out scale-105"
                style={{
                  left: `${pos.x}px`,
                  top: `${pos.y}px`,
              ...
              >
                ...
                <div className="absolute -top-2 -right-2 w-4 h-4 bg-primary rounded-full border-2 border-background shadow-lg animate-pulse" />
