...
import Canvas2DInterativo from "./Canvas2DInterativo";
...
export default function BoussinesqAnalise({ onVoltar }: BoussinesqAnaliseProps) {
  ...
  const [cargaP, setCargaP] = useState<number | undefined>(undefined);
  ...
  const [pontoPopupCoords, setPontoPopupCoords] = useState({ x: 0, z: 0 });
  ...
  const handleConfirmarPonto = (nome: string, x: number, z: number) => {
    if (pontoEditando) {
  ...
  const handleExportarExcel = async () => {
    ...
    const resultadosData = pontos.map(p => ({
      ...
      "X (m)": p.x.toFixed(2),
      "Z (m)": p.z.toFixed(2),
      "Δσz (kPa)": p.tensao !== undefined ? p.tensao.toFixed(4) : "N/A"
    }));
