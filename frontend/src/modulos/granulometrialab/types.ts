/**
 * Tipos e Schemas — Granulometria (Laboratório)
 * modulos/granulometrialab/types.ts
 */
...
export const PeneiramentoGrossoSchema = z.object({
  massa_total_umida: z.number().positive(),
  massa_total_seca: z.number().positive(),
  teor_umidade: z.number().nonnegative().optional(),
  massa_graos: z.number().positive(),
  peneiras: z.array(PeneiraDadoSchema).min(1),
});

export const PeneiramentoFinoSchema = z.object({
  massa_total_umida: z.number().positive().optional(),
  massa_total_seca: z.number().positive(),
  teor_umidade: z.number().nonnegative().optional(),
  massa_fina_seca: z.number().positive().optional(),
  peneiras: z.array(PeneiraDadoSchema).min(1),
  fator_n: z.number().positive().optional(),
});
