/**
 * Matemática Pura - Índices Físicos e Limites de Consistência
 * modulos/indiceslimites/calculos.ts
 */

import type { IndicesFisicosInput, IndicesFisicosOutput } from '@/lib/schemas/indices-fisicos';
import type { LimitesConsistenciaInput, LimitesConsistenciaOutput } from '@/lib/schemas/limites-consistencia';
...
export function calcularLimitesConsistencia(
  ...
  try {
    ...
    if (ll_calculado !== undefined && lp_calculado !== undefined) {
      ...
      if (ip_calculado < 0) ip_calculado = 0;
...
export function calcularIndicesFisicos(dados: IndicesFisicosInput): IndicesFisicosOutput {
  try {
    ...
    const mu_in = dados.peso_total; 
    const ms_in = dados.peso_solido; 
    const v_in = dados.volume_total; 
    ...
    if (w === undefined && mu_in !== undefined && ms_in !== undefined) {
      if (ms_in <= EPSILON) throw new Error('Massa/Peso seco não pode ser zero para calcular umidade.');
      if (mu_in < ms_in - EPSILON) throw new Error('Massa/Peso úmido menor que seco.');
      w = (mu_in - ms_in) / ms_in;
      mw_calc = mu_in - ms_in;
    ...
    if (gama_nat === undefined && mu_in !== undefined && v_in !== undefined) {
      if (v_in <= EPSILON) throw new Error('Volume total não pode ser zero.');
    ...
    if (gama_d === undefined && gama_nat !== undefined && w !== undefined) {
      if (1 + w <= EPSILON) throw new Error('Umidade (w) inválida (-100% ou menos)');
      gama_d = gama_nat / (1 + w);
    }

    if (gs === undefined && gama_s !== undefined) {
      if (gama_w <= EPSILON) throw new Error('Peso específico da água não pode ser zero.');
      gs = gama_s / gama_w;
    } else if (gama_s === undefined && gs !== undefined) {
      gama_s = gs * gama_w;
    } else if (gs !== undefined && gama_s !== undefined) {
      if (Math.abs(gama_s - gs * gama_w) > 1e-3 * gama_w) {
        return { erro: `Gs (${gs}) e Peso Esp. Sólidos (${gama_s}) inconsistentes para γw=${gama_w}.` };
      }
    }

    if (e === undefined && n !== undefined) {
      if (Math.abs(1 - n) <= EPSILON) throw new Error('Porosidade (n) não pode ser 100%');
      e = n / (1 - n);
    } else if (n === undefined && e !== undefined) {
      if (Math.abs(1 + e) <= EPSILON) throw new Error('Índice de Vazios (e) inválido');
      n = e / (1 + e);
    }

    if (e === undefined && gama_d !== undefined) {
      if (gs !== undefined) {
        if (gama_d <= EPSILON) e = gs > EPSILON ? Infinity : undefined;
        else {
          e = (gs * gama_w) / gama_d - 1;
          if (e < -EPSILON) throw new Error(`Índice de vazios negativo (${e.toFixed(4)}). Verifique γd e Gs.`);
          else if (e < 0) e = 0.0;
        }
      } else if (gama_s !== undefined) {
        if (gama_d <= EPSILON) e = gama_s > EPSILON ? Infinity : undefined;
        else {
          e = gama_s / gama_d - 1;
          if (e < -EPSILON) throw new Error(`Índice de vazios negativo (${e.toFixed(4)}). Verifique γd e γs.`);
          else if (e < 0) e = 0.0;
        }
      }
    }
    ...
    if (gs === undefined && gama_d !== undefined && e !== undefined) {
      if (Math.abs(gama_w) > EPSILON) {
        ...
        if (gs < 0) throw new Error('Cálculo resultou em Gs negativo.');
        if (gama_s === undefined) gama_s = gs * gama_w;
      }
    }
    ...
    if (e !== undefined && emax !== undefined && emin !== undefined) {
      if (e < emin - EPSILON || e > emax + EPSILON) {
        ...
        else if (Dr <= 35) classificacao_compacidade = 'Fofa';
        else if (Dr <= 65) classificacao_compacidade = 'Média';
        else if (Dr <= 85) classificacao_compacidade = 'Compacta';
        else classificacao_compacidade = 'Muito Compacta';
      }
    }

    if (S === undefined && w !== undefined && gs !== undefined && e !== undefined) {
      if (e <= EPSILON) S = 0.0; else S = (w * gs) / e;
      S = Math.max(0.0, Math.min(S, 1.0));
    } else if (w === undefined && S !== undefined && e !== undefined && gs !== undefined) {
      if (gs <= EPSILON) w = S * e <= EPSILON ? 0.0 : Infinity; else w = (S * e) / gs;
      if (w !== undefined && w < 0) w = 0.0;
    } else if (e === undefined && w !== undefined && gs !== undefined && S !== undefined) {
      if (S <= EPSILON) {
        if (w > EPSILON) throw new Error('Saturação (S) não pode ser 0 se a umidade (w) for maior que 0.');
      } else {
        e = (w * gs) / S;
        if (e < 0) throw new Error('Cálculo resultou em índice de vazios negativo.');
      }
    }
    ...
    if (gama_nat === undefined) {
      if (gama_d !== undefined && w !== undefined) gama_nat = gama_d * (1 + w);
    ...
    if (gama_d === undefined) {
      if (gama_nat !== undefined && w !== undefined) {
        if (Math.abs(1 + w) <= EPSILON) throw new Error('Umidade (w) inválida');
        gama_d = gama_nat / (1 + w);
    ...
    if (v_calc === undefined && mt_calc !== undefined && gama_nat !== undefined && gama_w > EPSILON) {
      ...
      if (gama_nat_gcm3_calc > EPSILON) v_calc = mt_calc / gama_nat_gcm3_calc;
    }

    if (ms_calc === undefined && mt_calc !== undefined && w !== undefined) {
      if (1 + w > EPSILON) {
        ms_calc = mt_calc / (1 + w);
        if (mw_calc === undefined) mw_calc = mt_calc - ms_calc;
      }
    ...
    if (vs_calc === undefined && ms_calc !== undefined && gs !== undefined && gama_w_gcm3 > EPSILON) {
      ...
      if (1 + e > EPSILON) vs_calc = v_calc / (1 + e);
    ...
    if (mu_in !== undefined && ms_in !== undefined && v_in !== undefined &&
        gs === undefined && e === undefined && n === undefined &&
        S === undefined && gama_s === undefined && dados.indice_vazios === undefined) {
      aviso = '⚠️ Cálculo parcial realizado. Para calcular TODOS os índices defina a Densidade Relativa dos Grãos (Gs).';
    }
    ...
    if (gs !== undefined) peso_s_norm = Number((gs * gama_w_gcm3 * vol_s_norm).toFixed(2));
    if (e !== undefined) {
      ...
      if (S !== undefined) {
        ...
        if (vol_w_norm !== undefined) peso_w_norm = Number((vol_w_norm * gama_w_gcm3).toFixed(2));
    ...
    return { erro: error instanceof Error ? error.message : 'Erro interno no cálculo' };
  }
}
...
export function calcularIndicesFisicosMultiplasAmostras(
  ...
  if (amostras.length === 0) return { erro: 'Nenhuma amostra fornecida' };
  ...
  if (amostras.length === 1) return { ...resultados[0], num_amostras: 1 };
