...
- **Status:** ✅ Concluído
- **Data:** 2026-03-25
- **Arquivos removidos (13 stubs soltos):**
  - `components/CalculationActions.tsx`, `ErrorBoundary.tsx`, `ExportPDFDialog.tsx`, `FeedbackMessage.tsx`, `OptimizedCard.tsx`, `PrintHeader.tsx`, `RecentCalculations.tsx`, `RoutePreloader.tsx`, `SaveDialog.tsx`, `SavedCalculations.tsx`, `UndoRedoToolbar.tsx`, `Layout.tsx`, `index.ts`
- **Diretórios de stubs removidos (34 stubs):**
  - `components/acrescimo-tensoes/` (18 stubs)
  - `components/compactacao/` (3 stubs)
  - `components/granulometria/` (4 stubs)
  - `components/limites/` (2 stubs)
  - `components/tensoes/` (7 stubs)
- **~50 imports atualizados em 15 arquivos:**
  - `App.tsx` — `Layout` → `layout/Layout`, `ErrorBoundary` → `shared/ErrorBoundary`
  - `Dashboard.tsx` — `RoutePreloader` → `shared/RoutePreloader`
  - `Compactacao.tsx`, `GranulometriaLab.tsx`, `TensoesGeostaticas.tsx`, `RecalqueAdensamento.tsx`, `AcrescimoTensoes.tsx`, `Granulometria.tsx` — `PrintHeader` → `layout/`, `SaveDialog` → `dialogs/`, `SavedCalculations` → `shared/`, `CalculationActions` → `shared/`, `ExportPDFDialog` → `dialogs/`
  - `modules/caracterizacao/index.tsx` — `PrintHeader`, `ExportPDFDialog`, `LimiteLiquidezChart`
  - `modules/acrescimoTensoes/components/` (4 arquivos: Boussinesq, Carothers, Love, Newmark)
  - Imports de `components/{acrescimo-tensoes,compactacao,granulometria,tensoes}/` → `modules/*/components/`
- **Diretórios vazios removidos:** `assets/`, `modules/caracterizacao/hooks/`
- **Observações:**
  - Componentes reais mantidos: `shared/`, `dialogs/`, `layout/`, `visualizations/`, `soil/`, `icons/`, `ui/`
  - TypeScript compila sem erros

### 2.2 Eliminação de Stubs em `lib/`
- **Status:** ⏳ Próximo
- **Data:**
- **Stubs identificados (5 re-exports):**
  - `lib/export-utils.ts` → `lib/utils/export-utils.ts` (9 imports)
  - `lib/tensoes-utils.ts` → `lib/utils/tensoes-utils.ts` (1 import)
  - `lib/format-number.ts` → `lib/utils/format-number.ts` (1 import)
  - `lib/peneiras-padrao.ts` → `lib/constants/peneiras-padrao.ts` (1 import)
  - `lib/soil-constants.ts` → `lib/constants/soil-constants.ts` (3 imports)
- **Nota:** `lib/utils.ts` NÃO é stub — contém `cn()` (tailwind-merge), usado amplamente
- **Observações:**

### 2.3 Eliminação de Stubs em `lib/calculations/`
- **Status:** ⏳ Próximo
- **Data:**
- **Stubs identificados (4 re-exports → modules):**
  - `lib/calculations/compactacao.ts` → `modules/compactacao/calculations` (1 import)
  - `lib/calculations/granulometria.ts` → `modules/granulometria/calculations` (1 import)
  - `lib/calculations/indices-fisicos.ts` → `modules/indicesFisicos/calculations` (1 import)
  - `lib/calculations/tensoes-geostaticas.ts` → `modules/tensoes/calculations` (1 import)
- **Arquivos REAIS em `lib/calculations/` (manter):**
  - `classificacao-uscs.ts` (12KB), `classificacao-hrb.ts` (8KB), `recalque-adensamento.ts` (10KB), `tempo-adensamento.ts` (4KB), `acrescimo-tensoes.ts`, `limites-consistencia.ts`, `index.ts`
- **Observações:****
